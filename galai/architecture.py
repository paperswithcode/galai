""" PyTorch OPT model. -- GALACTICA Adaptation"""
...
from galai.config import OPTConfig
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.self_attn_layer_norm = nn.LayerNorm(self.embed_dim, elementwise_affine=config.layer_norm_elementwise_affine) # force Marcin
        self.fc1 = nn.Linear(self.embed_dim, config.ffn_dim, bias=config.bias) # force Marcin
        self.fc2 = nn.Linear(config.ffn_dim, self.embed_dim, bias=config.bias) # force Marcin
        self.final_layer_norm = nn.LayerNorm(self.embed_dim, elementwise_affine=config.layer_norm_elementwise_affine) # force Marcin
